...
const Upload = () => {
  ...
  useEffect(() => {
    // List Puter models for debugging
    const puter = typeof window !== "undefined" && (window as any).puter;
    if (puter && puter.ai && typeof puter.ai.listModels === "function") {
      puter.ai.listModels()
        .then((models: any) => console.log("Puter Available Models:", models))
        .catch((err: any) => console.error("Error listing Puter models:", err));
    }
  }, []);
  ...
  const handleAnalyze = async ({
    ...
    try {
      ...
      try {
        ...
        return setStatusText(`Error: Failed to parse AI response. Response was: ${feedbackText.substring(0, 100)}...`);
      }
